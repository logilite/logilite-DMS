...
public class DefaultMountingStrategy implements IMountingStrategy
{
	...
	public MDMSContent getMountingParentForArchive(int AD_Table_ID, int Record_ID, int Process_ID)
	{
		...
		try
		{
			...
			else
			{
				...
				if (recordContentID <= 0)
				{
					...
					MDMSVersion.create(recordContentID, String.valueOf(Record_ID), 0, null, trxName);
				}
		...
		{
			trx.rollback();
			throw new AdempiereException("Default Parent Mounting Archive failed. Error: " + e.getLocalizedMessage(), e);
		}
		finally
		{
			trx.close();
