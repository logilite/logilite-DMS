...
public class MDMSMimeType extends X_DMS_MimeType
{
	...
	{
		int count = DB.getSQLValue(null, "SELECT COUNT(1) FROM DMS_MimeType WHERE UPPER(MimeType) = UPPER('"	+ getMimeType()
	...
	public static int getMimeTypeID(File file)
	{
		...
		if (file != null)
		{
			...
			if (!Util.isEmpty(ext))
				dmsMimeType_ID = DB.getSQLValue(null, SQL_GET_MIMETYPE_ID + " WHERE UPPER(FileExtension) = '" + ext.toUpperCase() + "'");
		}
	...
	public static MImage getThumbnail(int DMS_MimeType_ID)
	{
		...
		if (iconID <= 0)
		{
			...
			if (iconID <= 0)
				iconID = DB.getSQLValue(null, "SELECT AD_Image_ID FROM AD_Image WHERE UPPER(Name) = UPPER(?) ", DMSConstant.DOWNLOAD);
		}
